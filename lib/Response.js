...
Response.prototype.value = function () {
	...
	if (this.error) {
		...
		if (this.parsedPayload.isBase64Encoded) {
			this.parsedPayload.body = Buffer.from(this.parsedPayload.body, "base64");
		}
		if (this.config.responseType === "arraybuffer") {
			returnObject.data = this.parsedPayload.isBase64Encoded ? this.parsedPayload.body : Buffer.from(this.parsedPayload.body);
		} else {
			if (this.parsedPayload.isBase64Encoded) {
				this.parsedPayload.body = this.parsedPayload.body.toString();
			}
			try {
